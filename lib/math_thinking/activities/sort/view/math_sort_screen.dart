...
import '../view_model/math_sort_view_model.dart';

class MathSortScreen extends StatefulWidget {
  const MathSortScreen({super.key});

  @override
  State<MathSortScreen> createState() => _MathSortScreenState();
}

class _MathSortScreenState extends State<MathSortScreen> {
  late final MathSortViewModel _viewModel = MathSortViewModel();
  ...
  Future<void> _submitAnswer(String selected) async {
    await _ui.runGradedAnswerFlow(
      ...
      buildMathGradedSnapshotJson: (bool correct) {
        final MathSortQuestion? q = _viewModel.question;
        if (q == null) {
          return null;
        }
        return MathTraceSnapshot.sort(
          activityType: MathActivityType.sortNumbersAscDesc,
          isCorrect: correct,
  ...
  Future<void> _showExamResultDialog(BuildContext dialogContext) async {
    await MathActivityDialogs.showExamResult(
      ...
      onBackToPractice: () async {
        _viewModel.setSessionMode(MathSortSessionMode.practice);
        await _viewModel.nextQuestion();
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final MathSortQuestion? question = _viewModel.question;
    return PopScope(
      canPop: false,
                                ...
                                children: <Widget>[
                                  ...
                                  Text(
                                    question.direction ==
                                        ...
                                        ? l.mathSortInstructionAscending
                                        : l.mathSortInstructionDescending,
                                    textAlign: TextAlign.center,
